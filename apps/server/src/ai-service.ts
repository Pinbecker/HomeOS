...
import { loadAiPlanningContext } from './ai-context'
import { planAiCapture } from './ai-planner'
import { aiPlanSchema, type AiAction, type AiPlan } from './ai-schemas'
...
async function recordUpsert(entityType: string, entityId: string) {
  if (entityType === 'item') {
    const row = await db.query.items.findFirst({ where: eq(items.id, entityId) })
    if (row) await recordExternalChange({ entityType, entityId, operation: 'upsert', payload: row })
  } else if (entityType === 'list_item') {
    const row = await db.query.listItems.findFirst({ where: eq(listItems.id, entityId) })
    if (row) await recordExternalChange({ entityType, entityId, operation: 'upsert', payload: row })
  } else if (entityType === 'list') {
    const row = await db.query.lists.findFirst({ where: eq(lists.id, entityId) })
    if (row) await recordExternalChange({ entityType, entityId, operation: 'upsert', payload: row })
  } else if (entityType === 'record') {
    const row = await db.query.records.findFirst({ where: eq(records.id, entityId) })
    if (row) await recordExternalChange({ entityType, entityId, operation: 'upsert', payload: row })
  } else if (entityType === 'reminder') {
    const row = await db.query.reminders.findFirst({ where: eq(reminders.id, entityId) })
    if (row) await recordExternalChange({ entityType, entityId, operation: 'upsert', payload: row })
  }
}

async function recordDelete(entityType: string, entityId: string) {
  await recordExternalChange({ entityType, entityId, operation: 'delete', payload: null })
}
...
async function findList(action: AiAction, type: 'tasks' | 'shopping') {
  ...
  if (action.listName) {
    ...
    if (type === 'shopping') {
      ...
      await recordUpsert('list', listId)
      return await db.query.lists.findFirst({ where: eq(lists.id, listId) }) ?? null
    }
  }
...
async function findAssigneeId(action: AiAction, currentUser: SessionUser) {
  ...
  const userRows = await db.query.users.findMany({ columns: { id: true, name: true }, orderBy: [asc(users.name)] })
  return userRows.find(user => user.name.toLowerCase() === name)?.id
    ?? userRows.find(user => user.name.split(' ')[0]?.toLowerCase() === name)?.id
    ?? null
}

async function findRecordById(recordId: string | null) {
  if (!recordId) return null
  return db.query.records.findFirst({ where: eq(records.id, recordId), columns: { id: true, title: true, category: true } })
}

async function findEntityType(entityId: string | null): Promise<'record' | 'item' | null> {
  if (!entityId || entityId === 'TASK_PLACEHOLDER') return null
  const record = await db.query.records.findFirst({ where: eq(records.id, entityId), columns: { id: true } })
  if (record) return 'record'
  const item = await db.query.items.findFirst({ where: eq(items.id, entityId), columns: { id: true } })
  return item ? 'item' : null
}

function cleanFields(fields: RecordField[]) {
  return fields.map(field => ({ label: field.label.trim(), value: field.value.trim() })).filter(field => field.label || field.value)
}
...
async function linkRecordToItem(recordId: string, itemId: string, userId: string) {
  ...
  return { type: 'link_entities', entityType: 'entity_link', entityId: linkId, title: record.title }
}
...
async function clearShoppingList(action: AiAction) {
  ...
  if (!name) return { list: null, deletedIds: [], reason: 'Missing shopping list name.' }
  const list = rows.find(row => row.name.toLowerCase() === name || row.name.toLowerCase().includes(name))
  if (!list) return { list: null, deletedIds: [], reason: `Shopping list "${action.listName ?? action.title}" was not found.` }
  const itemRows = await db.query.listItems.findMany({ where: eq(listItems.listId, list.id), columns: { id: true } })
  await db.delete(listItems).where(eq(listItems.listId, list.id))
  for (const item of itemRows) await recordDelete('list_item', item.id)
  return { list, deletedIds: itemRows.map(item => item.id), reason: null }
}

async function applyAction(action: AiAction, user: SessionUser, jobId: string) {
  const now = new Date()
  const entityId = id(action.type)
  ...
  if (action.type === 'create_task' && action.title) {
    ...
    await recordUpsert('item', entityId)
    appliedActions.push({ type: action.type, entityType: 'item', entityId, title: action.title })
    if (action.recordId) {
      const link = await linkRecordToItem(action.recordId, entityId, user.id)
  ...
  if (action.type === 'create_note' && action.title) {
    ...
    await recordUpsert('item', entityId)
    return { appliedActions: [{ type: action.type, entityType: 'item', entityId, title: action.title }] }
  }
  ...
  if (action.type === 'create_shopping_item' && action.title) {
    ...
    await db.insert(listItems).values({ id: entityId, listId: list.id, title: action.title.trim(), sortOrder: Date.now(), checked: false, createdAt: now, updatedAt: now })
    await recordUpsert('list_item', entityId)
    return { appliedActions: [{ type: action.type, entityType: 'list_item', entityId, title: action.title, listId: list.id }] }
  }
  ...
  if (action.type === 'clear_shopping_list') {
    ...
    return { appliedActions: [{ type: action.type, entityType: 'list', entityId: result.list.id, title: result.list.name, deletedCount: result.deletedIds.length }] }
  }
  ...
  if (action.type === 'create_record' && action.recordCategory && action.recordTitle) {
    ...
    await recordUpsert('record', entityId)
    return { appliedActions: [{ type: action.type, entityType: 'record', entityId, title: action.recordTitle }] }
  }
  ...
  if (action.type === 'update_record' && action.recordId) {
    ...
    await recordUpsert('record', action.recordId)
  ...
  if (action.type === 'create_reminder') {
    ...
    const targetId = action.recordId ?? action.toEntityId ?? action.fromEntityId
    const record = await findRecordById(targetId)
    if (!targetId || !record) return { appliedActions: [], reason: 'Reminder target record was not found.' }
    ...
    await db.insert(reminders).values({ id: entityId, householdId: HOUSEHOLD_ID, createdById: user.id, entityType: RECORD_ENTITY_TYPE, entityId: targetId, message: action.reminderMessage ?? action.title, triggerAt, createdAt: now })
    await recordUpsert('reminder', entityId)
    return { appliedActions: [{ type: action.type, entityType: 'reminder', entityId, title: action.reminderMessage ?? action.title }] }
  }
  ...
  if (action.type === 'link_entities' && action.fromEntityId && action.toEntityId) {
    ...
    const linkId = id('link')
    await db.insert(entityLinks).values({ id: linkId, fromType, fromId: action.fromEntityId, toType, toId: action.toEntityId, linkType: 'related_to', createdById: user.id, createdAt: now })
    return { appliedActions: [{ type: action.type, entityType: 'entity_link', entityId: linkId }] }
  }
...
async function applySafeActions(plan: AiPlan, user: SessionUser, jobId: string) {
  ...
  if (plan.planningConfidence === 'low') {
    return { appliedActions: [], executionResults: plan.actions.map((action, index) => ({ index, type: action.type, attempted: false, status: 'skipped' as const, reason: 'Planning confidence was low.', appliedActions: [] })) }
  }
  ...
  for (const [index, action] of plan.actions.entries()) {
    if (action.confidence === 'low') {
      executionResults.push({ index, type: action.type, attempted: false, status: 'skipped', reason: 'Action confidence was low.', appliedActions: [] })
      continue
    }
    try {
      ...
      executionResults.push({ index, type: action.type, attempted: true, status: 'failed', reason: error instanceof Error ? error.message : 'Action failed.', appliedActions: [] })
    }
  }
...
async function createInboxItemForPlan(plan: AiPlan, user: SessionUser, jobId: string, finalResponse: string, originItemId?: string | null) {
  if (originItemId) {
    await db.update(items).set({ metadata: toAiMetadata(jobId, plan, finalResponse), updatedAt: new Date() }).where(eq(items.id, originItemId))
    await recordUpsert('item', originItemId)
    return null
  }
  ...
  await recordUpsert('item', entityId)
  return { id: entityId, title }
}

async function createConversationIfNeeded(plan: AiPlan, user: SessionUser, jobId: string, finalResponse: string, originItemId?: string | null, existingConversationId?: string | null, forceConversation = false) {
  ...
  if (existingConversationId) {
    ...
    await db.update(aiConversations).set({
      ...
      messages: [...(existing?.messages ?? []), { role: 'assistant', content: plan.clarificationQuestion || finalResponse, createdAt: now.toISOString() }],
      updatedAt: now,
    }).where(eq(aiConversations.id, existingConversationId))
    return existingConversationId
  }

  const conversationId = id('ai-conversation')
...
function actionVerb(action: AppliedAction) {
  ...
  if (type === 'clear_shopping_list') return `cleared ${action.title}${typeof action.deletedCount === 'number' ? ` (${action.deletedCount} item${action.deletedCount === 1 ? '' : 's'})` : ''}`
...
function buildFinalResponse(plan: AiPlan, appliedActions: AppliedAction[], executionResults: AiExecutionResult[]) {
  ...
  if (missingReminderDate && appliedActions.length === 0) {
    ...
    return `When should I remind you about ${action?.reminderMessage ?? action?.title ?? 'that'}?`
  }
  if (plan.result === 'needs_clarification') return plan.clarificationQuestion || plan.response
  if (plan.result !== 'apply_actions') return plan.response
  ...
  const done = summaries.length === 1 ? `I ${summaries[0]}.` : `I ${summaries.slice(0, -1).join(', ')} and ${summaries[summaries.length - 1]}.`
  const linkText = appliedActions.some(action => action.type === 'link_entities') && !done.includes('linked') ? ' I linked it as well.' : ''
  const skippedText = skipped.length > 0 ? ` I couldn't complete ${skipped.length === 1 ? 'one part' : `${skipped.length} parts`}: ${skipped.map(result => result.reason).filter(Boolean).join('; ')}` : ''
  return `${done}${linkText}${skippedText}`.trim()
}

export async function runAiCapture(request: AiCaptureRequest, user: SessionUser) {
  const now = new Date()
  const jobId = id('ai-job')
  ...
  try {
    const planning = await planAiCapture({
      rawInput: request.rawInput,
      context: await loadAiPlanningContext(user),
    ...
    const shouldCaptureInbox = plan.result === 'capture_to_inbox' || plan.result === 'unknown' || plan.result === 'needs_clarification' || appliedActions.length === 0
    const inboxItem = shouldCaptureInbox ? await createInboxItemForPlan(plan, user, jobId, finalResponse, request.originItemId) : null
    const conversationId = await createConversationIfNeeded(plan, user, jobId, finalResponse, request.originItemId ?? inboxItem?.id ?? null, request.conversationId, needsExecutionClarification)

    await db.update(aiJobs).set({
      status: statusForPlan(plan, appliedActions.length, executionResults),
      conversationId,
      relatedEntityIds: plan.relatedEntityIds,
    ...
    if (request.originItemId && plan.result === 'apply_actions' && appliedActions.length > 0) {
      await db.update(items).set({ status: 'archived', metadata: toAiMetadata(jobId, plan, finalResponse), updatedAt: new Date() }).where(eq(items.id, request.originItemId))
      await recordUpsert('item', request.originItemId)
    }

    if (conversationId && (request.originItemId || inboxItem?.id)) {
      const itemId = request.originItemId ?? inboxItem!.id
      await db.update(items).set({
        metadata: { ...toAiMetadata(jobId, plan), ai: { ...toAiMetadata(jobId, plan, finalResponse).ai, conversationId } },
        updatedAt: new Date(),
      }).where(eq(items.id, itemId))
      await recordUpsert('item', itemId)
    }

    return { jobId, conversationId, plan, finalResponse, appliedActions, executionResults, inboxItem }
  } catch (error) {
    await db.update(aiJobs).set({ status: 'error', classification: { error: error instanceof Error ? error.message : 'AI planning failed' }, updatedAt: new Date() }).where(eq(aiJobs.id, jobId))
    throw error
  }
}
...
export async function appendConversationUserMessage(conversationId: string, message: string) {
  const conversation = await db.query.aiConversations.findFirst({ where: eq(aiConversations.id, conversationId) })
  const messages = [
...
export async function conversationMessages(conversationId: string | null | undefined) {
  if (!conversationId) return []
  return (await db.query.aiConversations.findFirst({ where: eq(aiConversations.id, conversationId) }))?.messages ?? []
}
...
export async function confirmAiJob(jobId: string, user: SessionUser) {
  ...
  const { appliedActions, executionResults } = await applySafeActions({ ...plan, result: 'apply_actions' }, user, jobId)
  const finalResponse = buildFinalResponse(plan, appliedActions, executionResults)

  await db.update(aiJobs).set({
    status: appliedActions.length ? 'applied' : 'rejected',
    actionsTaken: appliedActions,
    executionResults,
    finalResponse,
  ...
  if (job.conversationId) {
    ...
    await db.update(aiConversations).set({
      status: appliedActions.length ? 'applied' : 'dismissed',
      messages: [...(conversation?.messages ?? []), { role: 'assistant', content: finalResponse, createdAt: new Date().toISOString() }],
      updatedAt: new Date(),
    }).where(eq(aiConversations.id, job.conversationId))
  }

  return { appliedActions, executionResults, finalResponse }
}
