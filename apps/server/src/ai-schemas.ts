...
export const aiPlanResultSchema = z.enum(['apply_actions', 'capture_to_inbox', 'needs_clarification', 'unknown'])
...
export const AI_PLAN_JSON_SCHEMA = {
  ...
  required: ['result', 'response', 'originalWording', 'planningConfidence', 'entityResolutionConfidence', 'inferredTags', 'relatedEntityIds', 'clarificationQuestion', 'clarificationOptions', 'confirmationSummary', 'actions'],
  properties: {
    result: { type: 'string', enum: ['apply_actions', 'capture_to_inbox', 'needs_clarification', 'unknown'] },
    ...
    actions: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['type', 'title', 'body', 'dueDate', 'listName', 'assigneeName', 'recordId', 'recordCategory', 'recordTitle', 'fields', 'reminderDate', 'reminderMessage', 'fromEntityId', 'toEntityId', 'confidence'],
        properties: {
          type: { type: 'string', enum: ['create_task', 'create_inbox_item', 'create_note', 'create_shopping_item', 'clear_shopping_list', 'create_record', 'update_record', 'create_reminder', 'link_entities'] },
